function [MRC ZF MMSE] = Dnointercell(d,link)
    ...
    for SNR = 1:length(Pt_dl)
        ...
        if contains(link,'dl')
            Pr_dl_dB(:,:,SNR) = get_Pr_ul(users,num_users,pl,shadow_var,Pt_dl_dB(SNR),ant);
    ...
    function [users,num_users, ant] = drop_users_ant(Nt) 
        ...
        r = 1 ;%km
        r_BS = 0.5; % radius at which BS antennas will fall in  
        ...
        ant = r_BS*sqrt((rand(Nt,1))).*exp(j*2*pi*(rand(Nt,1)));
        ...
        plot(users(in),'.')
        plot(ant,'p','MarkerSize',10,'MarkerFaceColor','r')
        hold off
    ...
    %     PL = 10*pl*log10(abs(users)) + sqrt(var)*randn(num_users,1)
        for k = 1:num_users
             PL = sqrt(var)*randn(1,1) - 10*pl*log10(abs(ant - users(k))) ;
             Pr_ul(:,k) = Pt - abs(PL);
