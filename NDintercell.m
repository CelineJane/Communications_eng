function [MRC ZF MMSE] = NDintercell(d,link)
    ...
    H_uncorr  = sqrt(1/2)*(randn(Nt,Nr,length(BS))+1i*randn(Nt,Nr,length(BS))); %rayleigh fading
    for k = 1:length(BS)
        H(:,:,k) = sqrtm(Rt)*H_uncorr(:,:,k)*sqrtm(Rr); % assume some correlation for non-distributed array
    end
    ...
    for SNR = 1:length(Pt_dl)
        if contains(link,'ul')
            Pr_ul_dB(:,:,SNR) = get_Pr_ul(users,num_users,pl,shadow_var,Pt_ul_dB(SNR),BS);
            Pr_ul(:,:,SNR) = 10.^(Pr_ul_dB(:,:,SNR)./10); 
            
            G = zeros(Nt,num_users,length(BS)) ; % channel matrix for all users
            for L = 1:length(BS)
                for k = 1:num_users
                    G(:,k,L) = sqrt(Pr_ul(L,k,SNR))*H(:,k,L);
                end
            end  
        ...
        if contains(link,'dl')
            Pr_dl_dB(:,:,SNR) = get_Pr_ul(users,num_users,pl,shadow_var,Pt_dl_dB(SNR),BS);
    ...
    %and drops users 
    function [users,num_users, BS] = drop_users() 
    ...
    function Pr_ul = get_Pr_ul(users,num_users,pl,var,Pt,BS) 
        for k = 1:num_users
            PL = -10*pl*log10(abs(BS - users(k,:))) + sqrt(var)*randn(1,1) ;
    ...
    function Pr_dl = get_Pr_dl(users,num_users,pl,var,Pt,BS) 
    %     PL = 10*pl*log10(abs(users)) + sqrt(var)*randn(num_users,1)
        for k = 1:num_users
             PL = -10*pl*log10(abs(BS - users(k,1))) + sqrt(var)*randn(1,1) ;
    %          pause(2)
             Pr_dl(:,k) = Pt - abs(PL);    
        end
            ...
            for n = 1:length(H) % user that is receiveing inteference from others
                for k = 1:length(H) %users giving interference           
                    if i == 1 & n == k %if the same user 
                        ...
                        intfrnce(k,n,i) = (abs(H(:,n,1)'*w(:,k,i))^2);
                    end
                end      
                sum_int_BS = sum(intfrnce);           
            end
    %         Pr(i,:)
            sum_int_BS(:,:,i) = nu(i).*Pr(i,:).*sum_int_BS(:,:,i);        
        end
       sum_total = sum(sum_int_BS,3);

       for n = 1:length(H)
           SINR(n) = ((Pt*nu(1)*Pr(1,n)*(norm(H(:,n,1)'*w(:,n,1))^2))/(((N0^2)+(Pt*sum_total(n)))))';
       end
